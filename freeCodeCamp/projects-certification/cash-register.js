...
function checkCashRegister(price, cash, cid) {
  ...
  // Evalúa si el total de lo que hay en caja es menor al vuelto, cuyo caso, 
  //retornamos el objeto inicializado (change);
  ...
  cid = cid.reverse().map((el) => {
    ...
    if (vuelto >= moneda) {
      ...
      // Evalúa si la cantidad de billetes de dicha moneda es menor o igual a lo que existe 
      // en caja, en cuyo caso se entregará. De lo contrario, se entregará el límite que haya en caja.
    ...
    // Se actualiza el indice en todos los casos, y si la moneda que está siendo evaluada, 
    // es superior a lo que hay que entregar en vuelto, pues se itera a la siguiente.
    index--;
    return el;
  });

  // Se evalua que lo restante de caja sea igual a cero. En cuyo caso, se cierra la caja y se 
  // actualiza el change del objeto, con lo que habia originalmente en caja, es decir, con arr2 (que se creó al inicio).
  ...
  // Se evalua que el vuelto sea igual a 0, porque si queda más vuelto que entregar, quiere 
  // decir que no había suficientes fondos. Por lo que se actualizará el objeto change.
